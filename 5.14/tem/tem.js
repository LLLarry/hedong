...
$(function () {
    ...
    $('.list-center1 .submit').click(function (e) { //点击修改电子模板提交/添加的电子模板提交
        ...
        var powerVal= $('.list-center1 input[name=power]').val().trim()
        if(nameVal.length <= 0){
        ...
        var flag= $('.list-center1 h3').html().trim() === '新增电子模板' ? true : false
        if(flag){ //新增电子模板
            //发送ajax将新增的数据传输到服务器=====================
            var str= '<p>显示名称：<span>'+nameVal+'</span></p><p>充电价格：<span>'+parseVal+' <b>元</b></span></p><p>充电时间：<span>'+timeVal+' <b>分钟</b></span></p> <p>消耗电量：<span>'+powerVal+'<b>度</b></span></p> <div> <button type="button" class="mui-btn mui-btn-success tem-edit">编辑</button> <button type="button" class="mui-btn mui-btn-success tem-delete">删除</button> </div>'
            ...
            parentEle.find('p').eq(2).find('span').html(timeVal)
            parentEle.find('p').eq(3).find('span').html(powerVal)
        }
        $('.tem-mask1').fadeOut()
    })
    ...
    function renderList(obj){ //渲染list-content
        ...
        $('.list-center1 input[name=power]').val(obj.power)
        $('.tem-mask1').fadeIn()
    }
